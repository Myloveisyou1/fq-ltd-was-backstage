...
public class ExcelUtils {
    ...
    private static final  String url = "C:\\Users\\fengqun\\Desktop\\我的下载\\";//本地上传地址
     ...
     * @return
     */
    public static String excel(Map<String,Object> param, String[] title, String excelName, String[][] content){
        ...
        sheet.addMergedRegion(new Region(0, (short) 0, 0, (short) (title.length-1)));//合并单元格，开始行，开始列，结束行，结束列
        cell1.setCellValue(excelName);
        cell1.setCellStyle(style1);

        HSSFRow row2 = sheet.createRow(1);//第二行      设置时间
        HSSFCell cell2 = row2.createCell((short) 0);
        sheet.addMergedRegion(new Region(1, (short) 0, 1, (short) (title.length-1)));//合并单元格，开始行，开始列，结束行，结束列
        cell2.setCellValue("统计时间："+param.get("time"));
        ...
        ExcelReadUtils.excelWrite(path,wb);
        return path;
    }
    ...
    public static void main(String[] args){
        ...
        map.put("startTime","2018-07-05");
        map.put("endTime","2018-07-06");

        String[] title = {"字段一","字段二","字段三","字段四","字段五","字段六"};
        String[][] content = new String[10][title.length];
        for(int i=0;i<10;i++){
        ...
        String path = excel(map,title,"测试",content);
